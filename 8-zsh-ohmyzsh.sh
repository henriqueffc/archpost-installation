...
if test -f "/home/$USER/.zshrc.pre-oh-my-zsh"; then
	...
	while :; do
		...
		s | S | "")
			...
			sudo pacman -Syu curl ttf-nerd-fonts-symbols-2048-em ttf-fira-code ttf-jetbrains-mono --needed --noconfirm
...
if test -f "/home/$USER/.p10k.zsh"; then
	...
	while :; do
		echo -ne "$VERDE Você quer instalar os plugins e o Powerlevel10k? Será habilitado também no .zshrc os aliases criados no arquivo ~/.bash_aliases. Plugins que serão instalados: zsh-autosuggestions, zsh-syntax-highlighting, colored-man-pages, copyfile, copybuffer, dirhistory, web-search, copypath, history-substring-search e zsh-completions. Será instalado também o tema de cores para Zsh Drácula. $FIM $LVERDE (S) sim / (N) não $FIM"
		read -r resposta
		case "$resposta" in
		s | S | "")
			echo -e "$AZUL Começando em 3... $FIM" && sleep 1
			...
			cp ~/.zshrc ~/.zshrc.bak
			sed -i 's/plugins=(git)/\plugins=(git zsh-autosuggestions zsh-syntax-highlighting colored-man-pages command-not-found universalarchive thefuck copyfile copybuffer dirhistory web-search copypath history-substring-search)/' ~/.zshrc
