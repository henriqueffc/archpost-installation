...
sudo sed -i '$ { s/^.*$/& quiet loglevel=3 systemd.show_status=auto rd.udev.log_level=3 modprobe.blacklist=iTCO_wdt nowatchdog intel_pstate=active intel_iommu=on iommu=pt pci=realloc,pcie_bus_perf processor.max_cstate=1 intel_idle.max_cstate=1 workqueue.power_efficient=0 nvidia_drm.modeset=1 nvidia.NVreg_EnablePCIeGen3=1 nvidia.NVreg_UsePageAttributeTable=1 nvidia.NVreg_DynamicPowerManagement=0x02 i915.nuclear_pageflip=1 i915.enable_guc=2 i915.enable_dc=0 i915.enable_fbc=0 i915.enable_psr=0 i915.edp_vswing=2 nouveau.modeset=0 snd_intel_dspcfg.dsp_driver=3 transparent_hugepage=madvise audit=1 audit_backlog_limit=500 lsm=landlock,lockdown,yama,integrity,apparmor,bpf/ }' /boot/loader/entries/*.conf
