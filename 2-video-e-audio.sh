...
sudo sed -i '$ { s/^.*$/& quiet loglevel=3 systemd.show_status=auto rd.udev.log_level=3 splash modprobe.blacklist=iTCO_wdt nowatchdog intel_pstate=active intel_iommu=on iommu=pt pci=realloc ahci.mobile_lpm_policy=1 processor.max_cstate=1 intel_idle.max_cstate=1 nvme_core.default_ps_max_latency_us=0 nvidia_drm.modeset=1 nvidia.NVreg_EnablePCIeGen3=1 nvidia.NVreg_UsePageAttributeTable=1 i915.nuclear_pageflip=1 i915.enable_guc=0 i915.enable_fbc=0 i915.enable_psr=0 i915.edp_vswing=2 nouveau.modeset=0 snd_intel_dspcfg.dsp_driver=3 transparent_hugepage=madvise audit=1 audit_backlog_limit=500 lsm=landlock,lockdown,yama,integrity,apparmor,bpf/ }' /boot/loader/entries/*.conf
