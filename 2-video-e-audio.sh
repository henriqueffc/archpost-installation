...
sudo sed -i '$ { s/^.*$/& nowatchdog intel_pstate=hwp_only intel_iommu=on pci=realloc intel_idle.max_cstate=1 nvme_core.default_ps_max_latency_us=0 nvidia-drm.modeset=1 nvidia.NVreg_EnablePCIeGen3=1 nvidia.NVreg_UsePageAttributeTable=1 i915.modeset=1 i915.edp_vswing=2 i915.enable_dc=0 i915.enable_guc=2 i915.enable_fbc=0 i915.enable_psr=0 nouveau.modeset=0 audit=1 audit_backlog_limit=500 lsm=landlock,lockdown,yama,integrity,apparmor,bpf/ }' /boot/loader/entries/*.conf

# Não seria necessário o i915.modeset=1, pois já habilitei o early KMS no mkinitcpio.conf (script 1). Mas optei por deixar como parâmetro do kernel também. 
